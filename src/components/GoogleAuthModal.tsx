...
import { Cloud, LogIn, LogOut, CheckCircle2, ShieldCheck, Key } from 'lucide-react';
...
export const GoogleAuthModal: React.FC<GoogleAuthModalProps> = ({
  ...
  const triggerGoogleGISLogin = () => {
    setError(null);
    if (typeof window !== 'undefined' && window.google?.accounts?.oauth2) {
      try {
        // Attempt token client
        const client = window.google.accounts.oauth2.initTokenClient({
          client_id: '78747068045-preview.apps.googleusercontent.com',
        ...
        setError(err.message || 'Error launching Google OAuth popup.');
      }
    } else {
      setError('Google Identity Services script is still loading. You can also paste an OAuth access token below.');
    }
  };
          ...
          {driveUser.isSignedIn ? (
            ...
            <div className="space-y-4">
              <p className="text-xs text-slate-600 leading-relaxed">
                Connect your Google account to automatically back up generated PDF invoices to your Google Drive. We only request file access to store invoices created inside this app.
              </p>
              ...
              <button
                onClick={triggerGoogleGISLogin}
                disabled={loading}
                className="w-full flex items-center justify-center gap-3 py-2.5 px-4 rounded-xl border border-slate-300 bg-white hover:bg-slate-50 text-slate-700 font-semibold text-xs shadow-xs transition"
              >
              ...
              {/* Developer / Direct OAuth Token option */}
