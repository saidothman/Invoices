...
import { Building2, Landmark, Upload, Check, AlertCircle, RefreshCw } from 'lucide-react';
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-w-4xl">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-200">
        <div>
          <h2 className="text-xl font-bold text-slate-900">Business Profile & Banking</h2>
          <p className="text-xs text-slate-500 mt-0.5">
            Configure your logo, business address, and banking transfer credentials for invoice headers.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200 space-y-4">
          <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
            ...
            <span>Company Branding & Address</span>
          </h3>
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-700 mb-1">City, State, Zip & Country</label>
            <input
              type="text"
              value={formData.cityStateZip}
              onChange={(e) => handleChange('cityStateZip', e.target.value)}
              placeholder="e.g. San Francisco, CA 94107, USA"
              className="w-full text-xs px-3 py-2 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none"
            />
          </div>
        </div>
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200 space-y-4">
          <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
            ...
            <span>Bank Remittance & Settlement Info</span>
          </h3>

          <p className="text-xs text-slate-500">
            These wire/remittance instructions are clearly printed on every issued invoice for your clients.
          </p>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1">Routing / IBAN</label>
              <input
                type="text"
                value={formData.routingOrIban}
                onChange={(e) => handleChange('routingOrIban', e.target.value)}
                placeholder="e.g. 121000358 or US89..."
                className="w-full text-xs font-mono px-3 py-2 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none"
              />
            </div>
          </div>
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-700 mb-1">SWIFT / BIC Code (for International)</label>
            <input
              type="text"
              value={formData.swiftBic}
              onChange={(e) => handleChange('swiftBic', e.target.value)}
              placeholder="e.g. SVBKUS33XXX"
              className="w-full text-xs font-mono px-3 py-2 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none"
            />
          </div>

          <div>
            <label className="block text-xs font-semibold text-slate-700 mb-1">Default Payment Terms & Policy</label>
            <textarea
              rows={2}
              value={formData.paymentTermsNote}
              onChange={(e) => handleChange('paymentTermsNote', e.target.value)}
              placeholder="e.g. Payment due Net 14 days of invoice receipt. Wire fees paid by remitter."
              className="w-full text-xs px-3 py-2 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none"
            />
          </div>
        </div>
